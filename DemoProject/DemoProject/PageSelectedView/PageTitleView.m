...
#define kNormalColor @{@"red":@0, @"green":@0, @"blue":@0}
...
- (void)setupScrollLine
{
    ...
    CGFloat scrollLineX = fristLable.frame.origin.x + (fristLable.frame.size.width - fristLable.frame.size.width) / 2;
    self.scrollLine = [[UIView alloc] initWithFrame:CGRectMake(scrollLineX, fristLable.frame.size.height - kScrollLineH, fristLable.frame.size.width, kScrollLineH)];
...
- (void)lableTapClick:(UITapGestureRecognizer *)gesture
{
    ...
    [UIView animateWithDuration:kDurationTime animations:^{
        CGRect tempRect = self.scrollLine.frame;
        tempRect.origin.x = currentlabel.center.x - currentlabel.frame.size.width / 2;
        self.scrollLine.frame = tempRect;
    }];
